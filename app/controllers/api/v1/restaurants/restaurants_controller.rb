...
class Api::V1::Restaurants::RestaurantsController < Api::V1::BaseController
  ...
  def index
    render json: {'restaurants': helpers.get_restaurants}
  end

  def menu
    # Create hash of menu section's ms_name => { hash of food in section }
    menu_hash = {}
    menu_sections = helpers.get_menu_sections(params[:id]).to_a

    menu_sections.each do |section|
      menu_hash[section["ms_name"]] = helpers.get_section_food(params[:id], section["url_id"])
    end
